    ...
    if(optimized_rasterization)
    {
        ...
        if(vertex_is_clipped != 0xff)
        {
            ...
            for(unsigned int bk0 = 0 ; bk0 < 3 ; bk0++)
            {
                ...
                {
                    custom_cam.blackface_culling_enabled = true;
                    int bk0_i = (1 - (int)face_sign) / 2;
                    for(unsigned int bk1_i = 0 ; bk1_i < 2 ; bk1_i++)
                    {
                        for(unsigned int bk2_i = 0 ; bk2_i < 2 ; bk2_i++)
                        {
                            int i = (bk0_i << bk0) | (bk1_i << bk1) | (bk2_i << bk2);
                            if((vertex_is_clipped & (1<<i)) != 0)
                            {
                                custom_cam.blackface_culling_enabled = false;
                            }
                        }
                    }
